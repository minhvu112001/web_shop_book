...
$url_delete = urlAdminAuthorDelete().$url;
...
foreach ($results as $result) 
{
		...
		// thì dùng hình rỗng đã được đặt sẵn trong thư mục ảnh DIR_IMAGE
			$authors[] = array(
				...
				'edit'       => urlAdminAuthorEdit() . '?author_id=' . $result['author_id']
			);
}
 ...
 */
$sort_name       = urlAdminAuthor() . '?sort=name' . $url;
$sort_author_id  = urlAdminAuthor() . '?sort=author_id' . $url;
$sort_sort_order = urlAdminAuthor() . '?sort=sort_order' . $url;
...
paginate($author_total, $page,settings('config_limit_admin'), urlAdminAuthor().$url);

// Nội dung riêng của trang...
$web_title = "Tác Giả";
$web_content = "../ui/admin/view/view-author-list.php";
$active_page_admin = ACTIVE_PAGE_ADMIN_AUTHOR; 
