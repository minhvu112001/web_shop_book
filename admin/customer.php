...
foreach ($results as $result) 
{
	$customers[] = array(
		...
		'edit'        => urlAdminCustomerEdit()."?customer_id=".$result['customer_id'].$url
	);
}
...
$sort_email      = urlAdminCustomer()."?sort=email".$url;
$sort_status     = urlAdminCustomer()."?sort=status".$url;
$sort_date_added = urlAdminCustomer()."?sort=date_added".$url;
...
paginate($total_user, $page,settings('config_limit_admin'), urlAdminCustomer().$url);
