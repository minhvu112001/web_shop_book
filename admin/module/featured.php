...
include_once '../module-validate.php';
		...
		{ 
			if (!isset($_GET['module_id'])) {
				moduleAdd('featured', $_POST);
			} else {
				moduleEdit($_GET['module_id'], $_POST);
			}
//						
			$_SESSION['SUCCESS_TEXT'] = 'Bạn đã sửa thành công module sản phẩm nổi bật';
//
		...
		if (!isset($_GET['module_id'])) {
			...
			$breadcrumbs[] = array(
				'text' => 'Sản Phẩm Nổi Bật',
				'href' => urlAdminModuleFeatured() . '?module_id=' . $_GET['module_id']
			);			
		}

		if (!isset($_GET['module_id'])) {
			$url_action = urlAdminModuleFeatured();
		} else {
			$url_action = urlAdminModuleFeatured() . '?module_id=' . $_GET['module_id'];
		}
		...
		// Chiều cao ảnh sản phẩm nổi bật	
...
$web_title = 'Module-Sản phẩm nổi bật';
 
// Chỉ định nội dung riêng cho trang này 
$web_content = "../../ui/admin/view/view-module-featured.php";
