...
function search() 
{
	...
	// tham số truy vấn
	// ví dụ: http://localhost:82/web/book/search.php?search=iphone
	if (value) {
...
// thì họ dùng hàm function addToCart(book_id, quantity) chứ ko dùng OOP.
//
...
{ 
	$.ajax({
		...
		success: function(json) {	
			...
			if (json['redirect']) {
					// error books are to be redirected ?
...
function cartAdd(book_id, quantity) { addToCart(book_id, quantity); }

function updateCart(key, quantity) 
{
		$.ajax({
			url: $('#urlCartEdit').attr('href'),
			type: 'post',
			...
			success: function(json) {
				...
				if ( window.location.pathname == $('#urlCart').attr('href'))
				{ 
