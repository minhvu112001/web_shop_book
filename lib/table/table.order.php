...
function orderAdd($data) 
{
	...
	if (isset($data['books'])) 
	{
			foreach ($data['books'] as $book) 
			{
				...
				$name       = db_escape($book['name']); 
				$model      = db_escape($book['model']); 
				$quantity   = (int)$book['quantity']; 
				$price      = (float)$book['price']; 
				$total      = (float)$book['total']; 
...
function orderDetailsWithFormat($order_id)
{
	...
	if (is_array($order)) 
	{
		...
		foreach (orderGetBooks($order_id) as $book)
		{
			...
			$order['books'][] = array(
					...
					'price'    => $book_price = number_format($book['price'],0,'.',',').' ₫',
					'total'    => $book_price = number_format($book['total'],0,'.',',').' ₫'
			);
		}
	}
...
function orderGetBooks($order_id) 
{
...
function orderGetLatestForDashboard()
{
	...
	foreach ($results as $result) 
	{
				$orders[] = array(
					...
					'view'          => urlAdminOrderInfo() . '&order_id=' . $result['order_id']
				);
	}
