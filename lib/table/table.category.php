...
function categoryDelete($category_id) 
{
			...
			db_q("DELETE FROM `book_to_category` WHERE `category_id` = {$category_id}");
...
/**
 * @description Đếm tổng số loại sản phẩm
 * @return
 * a number if successfully queried
 * 0 if there are no records
 * NULL if there is error with query (Ex: SELECT COUNT(*) FROM categoryFuck WHERE 3 < 1)
 */
	...
	foreach ($categories as $category) 
	{
				if ($category['top']) 
				{
					...
					foreach ($children as $child) 
					{
						...
						$children_data[] = array(
							'name'  => $child['name'] . ' ('. bookGetTotalForCategory($child['category_id']) . ')',
							'href'  => urlBookCategory() . '?path=' . $category['category_id'] . '_' . $child['category_id']
						);
					}
					...
					$categoriesMenu[] = array(
						...
						'href'     => urlBookCategory() . '?path=' . $category['category_id']
					);
				}
	}
...
/**
 * Lấy ra danh sách các loại sản phẩm gắn liền với một module cụ thể.
 * Ví dụ các loại sản phẩm được treo trên một banner.
 * example module code: 'banner_categories'
 * @param unknown $module
 */
function categoryGetByModule($module)
{
	...
	foreach ($categories as $category_id)
	{
		...
		if ($category_info) {
			if ($category_info['image']) {
				$image = img_resize($category_info['image'], $setting['width'], $setting['height']);
			} else {
				$image = img_resize('placeholder.png', $setting['width'], $setting['height']);
			}

			$categoriesForModule[] = array(
					'category_id'  => $category_info['book_id'],
					'thumb'       => $image,
					'name'        => $category_info['name'],
					'description' => utf8_substr(strip_tags(html_entity_decode($category_info['description'], ENT_QUOTES, 'UTF-8')), 0, settings('config_book_description_length')) . '..',
					'href'        => urlBookCategory().'?path=' . $category_info['category_id'],
					'width'       => $setting['width'],
